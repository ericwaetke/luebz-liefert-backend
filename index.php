...
class NotificationHandler{
	function sendNotifications($unternehmenId, $kategorien, $header, $slug, $url){
		...
		$unternehmensql = "SELECT subscriptions.adress, subscriptions.auth, subscriptions.p256dh
				FROM subscriptions
		...
		$result = $connection->query($unternehmensql);

		$meldungen = [];

		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				array_push($meldungen, [
		...
		foreach ($meldungen as $notification) {
